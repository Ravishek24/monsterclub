...
const listMember = async (req, res) => {
    ...
    const [users] = await connection.query(`SELECT * FROM users WHERE veri = 1 AND proxy = 0 AND level != 2 ORDER BY id DESC LIMIT ${pageno}, ${limit} `);
    const [total_users] = await connection.query(`SELECT * FROM users WHERE veri = 1 AND proxy = 0 AND level != 2`);
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas: users,
...
const listCTV = async (req, res) => {
    ...
    const [wingo] = await connection.query(`SELECT * FROM users WHERE veri = 1 AND proxy = 0 AND level = 2 ORDER BY id DESC LIMIT ${pageno}, ${pageto} `);
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas: wingo,
...
const rechargeBonuses = async (req, res) => {
    const auth = req.cookies.auth;
    // const recharge_f = parseFloat(req.body.recharge_f) || 0;
    // const recharge_p = parseFloat(req.body.recharge_p) || 0;
    const joiningBonus = parseInt(req.body.joiningBonus) || 0;
    const timeNow = new Date().toISOString(); // Define the timestamp
    ...
    try {
        const [data] = await connection.query('SELECT recharge_bonus_2, recharge_bonus, signup_bonus FROM admin WHERE id = 1');
        const currentAdmin = data[0]; 

        // if (recharge_f !== 0 && recharge_f !== currentAdmin.recharge_bonus) {
        //     await connection.query('UPDATE admin SET recharge_bonus = ? WHERE id = 1', [recharge_f]);
        // }
        // if (recharge_p !== 0 && recharge_p !== currentAdmin.recharge_bonus_2) {
        //     await connection.query('UPDATE admin SET recharge_bonus_2 = ? WHERE id = 1', [recharge_p]);
        // }
        if (joiningBonus !== 0 && joiningBonus !== currentAdmin.signup_bonus) {
            await connection.query('UPDATE admin SET signup_bonus = ? WHERE id = 1', [joiningBonus]);
        }

        return res.status(200).json({
            message: 'Success! Data updated',
            data: {
                // recharge_bonus: recharge_f || currentAdmin.recharge_bonus,
                // recharge_bonus_2: recharge_p || currentAdmin.recharge_bonus_2,
                signup_bonus: joiningBonus || currentAdmin.signup_bonus,
            },
            status: true,
        });
