...
const userInfo = async (req, res) => {
    let auth = req.cookies.auth;
    const timeNow = new Date().toISOString(); // Ensure that timeNow is defined.
    ...
    const user = rows[0]; // Extract user data from rows

    // Fetch recharge and withdrawal data
    const [rechargeData] = await connection.query('SELECT * FROM recharge WHERE `phone` = ? AND status = 1', [user.phone]);
    let totalRecharge = 0;
    rechargeData.forEach((data) => {
        totalRecharge += data.money;
    });

    const [withdrawData] = await connection.query('SELECT * FROM withdraw WHERE `phone` = ? AND status = 1', [user.phone]);
    let totalWithdraw = 0;
    ...
    // Fetch recharge and minutes_1 data for calculations
    const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = 1', [user.phone]);
    const [minutes_1] = await connection.query('SELECT * FROM minutes_1 WHERE phone = ?', [user.phone]);

    let total = 0;
    recharge.forEach((data) => {
        total += parseFloat(data.money); // Total recharge amount
    });

    let total2 = 0;
    minutes_1.forEach((data) => {
        total2 += parseFloat(data.money); // Total minutes spent
    });

    let fee = 0;
    minutes_1.forEach((data) => {
        fee += parseFloat(data.fee); // Total fee
    });

    // Ensure that result is non-negative
    let result = Math.max(total - total2 - fee, 0); // Calculate result (remaining balance after fee deduction)
...
const userphoto = async (req, res) => {
    ...
    if (!auth || !url) {
        return res.status(400).json({
            message: 'Failed: Missing auth token or URL',
            status: false,
            timeStamp: timeNow,
        });
    }
    ...
    try {
        await connection.query('UPDATE users SET img_user = ? WHERE token = ?', [url, auth]);

        return res.status(200).json({
            message: 'User photo update successful',
            status: true,
            timeStamp: timeNow,
        });
    } catch (error) {
...
const promotion = async (req, res) => {
    ...
    let rosesAdd = rosesF1 + rosesAll;
     ...
     // Fetch direct data
        const [direct] = await connection.query('SELECT * FROM `users` WHERE `invite` = ?', [userInfo.code]);

        // Fetch recharge data for direct users
        const alldata = [];
        await Promise.all(direct.map(async (data, index) => {
            try {
                const [result] = await connection.query(`
                    SELECT phone, money, today, time 
                    FROM recharge 
                    WHERE phone = ? AND DATE(today) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)
                `, [parseInt(data.phone)]);
        ...
        // Fetch direct yesterday data
        let directyesterday;
        try {
            [directyesterday] = await connection.query('SELECT * FROM `users` WHERE `invite` = ? And DATE(FROM_UNIXTIME(time / 1000)) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)', [userInfo.code]);
        } catch (error) {
            console.error('Error fetching direct yesterday data:', error);
        }
        // Fetch team data
        const allPhonesNested = direct.map(item => item.code);
        const teamdata = await Promise.all(allPhonesNested.map(async (phone) => {
            try {
                return await connection.query('SELECT * FROM `users` WHERE `invite` = ?', [phone]);
            } catch (error) {
        ...
        const newarray = teamdata.map(item => item[0]);
        // Fetch recharge data for team members
        const pickPhone = newarray.reduce((acc, val) => acc.concat(val), []);
        const dataPhone = await Promise.all(pickPhone.map(async (item) => {
            try {
                const [result] = await connection.query(`
                    SELECT phone, money, today, time 
                    FROM recharge  
                ...
                console.error('Error fetching recharge data for team member:', error);
                return [];
            }
        }));
        
        const [usercodetodayjoin] = await connection.query('SELECT * FROM users WHERE `invite` = ? AND DATE(`today`) = CURDATE()', [userInfo.code]);
        const newtoday = usercodetodayjoin.length;
        
        const [todayjoin] = await connection.query('SELECT * FROM users WHERE DATE(`today`) = CURDATE()');
        const allnewtoday = todayjoin.length;
        
        const [roses_yest] = await connection.query('SELECT SUM(f1) as am FROM roses WHERE DATE(FROM_UNIXTIME(time / 1000)) = DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND phone = ?', [userInfo.phone]);
...
const promotionrecord = async (req, res) => {
    const auth = req.cookies.auth;
    const type = req.body.type;
    const timeNow = new Date().toISOString(); // Dynamically calculate the timestamp.
    ...
    try {
        // Fetch user details based on the provided token.
        ...
        // Helper function to fetch promotion records based on the type of filter.
        const getPromotionResults = async (filterQuery, filterParams) => {
            const [promotionResults] = await connection.query(filterQuery, filterParams);
        ...
        if (type == 0) {
            // Today's promotion results
            promotionResults = await getPromotionResults(
                'SELECT id, phone, id_user, today FROM `users` WHERE DATE(`today`) = CURDATE() AND `invite` = ?',
                [userInfo.code]
            );
        } else if (type == 1) {
            // Yesterday's promotion results
            promotionResults = await getPromotionResults(
                'SELECT id, phone, id_user, today FROM `users` WHERE DATE(`today`) = DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND `invite` = ?',
                [userInfo.code]
            );
        } else if (type == 2) {
            // Current month's promotion results
            promotionResults = await getPromotionResults(
                'SELECT id, phone, id_user, today FROM `users` WHERE YEAR(`today`) = YEAR(CURDATE()) AND MONTH(`today`) = MONTH(CURDATE()) AND `invite` = ?',
                [userInfo.code]
            );
        }
        ...
        const formattedResults = promotionResults.map(record => {
            return {
                ...record,
        ...
        // Send the response with the fetched promotion results.
        return res.status(200).json({
            message: 'Success',
            status: true,
            timeStamp: timeNow,
...
const recharge = async (req, res) => {
    ...
    if (recharge.length == 0) {
        ...
        const apiData = {
            ...
            redirect_url: `https://Damantrade.online/wallet/verify/upi`,
            udf1: 'TIRANGA',
        };
...
const addBank = async (req, res) => {
    ...
    if (!auth || !name_bank || !name_user || !stk || !stk || !tinh ) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
            timeStamp: timeNow,
    ...
    if (user_bank.length == 0 && user_bank2.length == 0) {
        ...
        time = ?`;
        await connection.execute(sql, [userInfo.phone, name_bank, name_user, stk, sdt, tinh, time]);
        return res.status(200).json({
        ...
        await connection.query('UPDATE user_bank SET name_bank = ?, name_user = ?, stk = ?, sdt = ?, tinh = ?, time = ? WHERE phone = ?', [name_bank, name_user, stk, sdt, tinh, time, userInfo.phone]);
        return res.status(200).json({
    ...
    if (withdraw.length < 3) {
        if (userInfo.money - money >= 0) {
            if (result == 0) {
                ...
                today = ?,
                time = ?`;
                    await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, money, infoBank.stk, infoBank.name_bank, infoBank.sdt, infoBank.name_user, 0, checkTime, dates]);
...
const getRechargelist = async (req, res) => {
    ...
    try {
        ...
        let list = (recharge.length > 0) ? recharge : {};

        const lists = recharge.map(record => ({
            addTime: record.today,
            payID: record.id_order,
            price: record.money,
            rechargeNumber: record.phone,
            state: record.status,
            type: record.type,
        }));
...
const listWithdraw = async (req, res) => {
    ...
    if(type == 'all') {
        const [withdraw] = await connection.query(
            'SELECT * FROM withdraw WHERE phone = ? ORDER BY id DESC LIMIT ?, ?',
            [userInfo.phone, (page - 1) * limit, limit]
          );

        let list = (withdraw.length > 0) ? withdraw : {};
        ...
        const [withdraw] = await connection.query('SELECT * FROM withdraw WHERE phone = ?  &&name_bank = ? ORDER BY id DESC ', [userInfo.phone, type]);
...
const gamestatis = async (req, res) => {
    const auth = req.cookies.auth;
    const type = req.body.type;
...
const messagelist = async (req, res) => {
    ...
    try {
        ...
        await connection.query(
                'UPDATE login_notification SET status = 1 WHERE status = 0 AND phone = ?',
                [userInfo.phone]
            );
        const [withdraw] = await connection.query(
            'SELECT * FROM login_notification WHERE phone = ? AND (status = 0 OR status = 1) ORDER BY id DESC LIMIT ?, ?',
            [userInfo.phone, (page - 1) * limit, limit]
        );
...
const trnsactiontype = async (req, res) => {
    let auth = req.cookies.auth;
    let type = req.body.type;
    ...
    if (type == "0") {
        const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND DATE(today) = ?  ORDER BY today DESC', [userInfo.phone, date]);
        const [withdraw] = await connection.query('SELECT * FROM withdraw WHERE phone = ? AND DATE(today) = ?  ORDER BY today DESC', [userInfo.phone, date]);
        const [giftcode] = await connection.query('SELECT * FROM redenvelopes_used WHERE phone_used = ? AND DATE(FROM_UNIXTIME(`time` / 1000)) = ? ORDER BY time DESC', [userInfo.phone, date]);
        const [bet] = await connection.query('SELECT * FROM minutes_1 WHERE phone = ? AND DATE(today) = ?  ORDER BY today DESC', [userInfo.phone, date]);
        const [salary] = await connection.query('SELECT * FROM salary WHERE phone = ? AND DATE(time) = ? ORDER BY time DESC', [userInfo.phone, date]);

        const list = [
            ...recharge.map(item => ({ ...item, typeName: 'Deposit' })),
            ...withdraw.map(item => ({ ...item, typeName: 'Withdraw' })),
            ...giftcode.map(item => ({ ...item, typeName: 'Giftcode' })),
            ...bet.map(item => ({ ...item, typeName: 'Bet' })),
            ...salary.map(item => ({ ...item, typeName: 'Salary' })),
        ];

        const sortedList = list.sort((a, b) => new Date(b.today) - new Date(a.today));

        return res.status(200).json({
            message: 'Receive success',
            datas: { lists: sortedList },
            status: true,
            timeStamp: Date.now(),
        });
    } else {
        ...
        if (type == "1") {
            query = 'SELECT * FROM minutes_1 WHERE phone = ? AND status = 2 AND DATE(today) = ? ORDER BY today DESC';
            typename = 'Bet';
        } else if (type == "2") {
            query = 'SELECT * FROM minutes_1 WHERE phone = ? AND status = 1 AND DATE(today) = ?  ORDER BY today DESC';
            typename = 'Win';
        } else if (type == "3") {
            query = 'SELECT * FROM redenvelopes_used WHERE phone_used = ? AND DATE(FROM_UNIXTIME(`time` / 1000)) = ?  ORDER BY time DESC';
            typename = 'Red envelops';
        } else if (type == "4") {
            query = 'SELECT * FROM recharge WHERE phone = ? AND status = 1 AND DATE(today) = ?  ORDER BY today DESC';
            typename = 'Deposit';
        } else if (type == "5") {
            query = 'SELECT * FROM withdraw WHERE phone = ? AND status = 1 AND DATE(today) = ?  ORDER BY today DESC';
            typename = 'withdraw';
        } else if (type == "6"){
            query = 'SELECT * FROM recharge WHERE phone = ? AND status = 0 AND DATE(today) = ?  ORDER BY today DESC';
            typename = 'Pending';
        } else if (type == "7"){
            query = 'SELECT * FROM recharge WHERE phone = ? AND status = 2 AND DATE(today) = ?  ORDER BY today DESC';
            typename = 'Reject';
        } else if (type == "8"){
            query = 'SELECT * FROM withdraw WHERE phone = ? AND status = 0 AND DATE(today) = ?  ORDER BY today DESC';
            typename = 'Pending';
        } else if (type == "9"){
            query = 'SELECT * FROM withdraw WHERE phone = ? AND status = 2 AND DATE(today) = ?  ORDER BY today DESC';
            typename = 'Reject';
        } else if (type == "10"){
            query = 'SELECT * FROM salary WHERE phone = ? AND DATE(time) = ?  ORDER BY time DESC';
            typename = 'Salary';
        } else {
            return res.status(400).json({
        ...
        const [data] = await connection.query(query, [userInfo.phone, date]);

        const list = data.map(item => ({ ...item, typeName: typename }));

        const sortedData = list.sort((a, b) => new Date(b.time) - new Date(a.time));

        return res.status(200).json({
            message: 'Receive success',
            datas: { lists: sortedData },
            status: true,
            timeStamp: Date.now(),
        });
    }
...
const getrebatedetail = async (req, res) => {
    let auth = req.cookies.auth;
    ...
    if (!userInfo) {
        ...
        const [bet] = await connection.query('SELECT SUM(money) as money FROM minutes_1 WHERE phone = ?  ORDER BY id DESC', [userInfo.phone]);
        const [bet1] = await connection.query('SELECT * FROM minutes_1 WHERE phone = ? ORDER BY id DESC LIMIT 5', [userInfo.phone]);

        return res.status(200).json({
            message: 'Receive success',
            datas: { codewashamount: bet[0].money },
            status: true,
            timeStamp: Date.now(),
        });
};
...
const bethistory = async (req, res) => {
    ...
    const page = parseInt(req.body.page);
    const limit = parseInt(req.body.limit);
    const timeNow = new Date().toISOString(); // Set timeNow to current timestamp
    ...
    try {
        ...
        // Determine the query and game title based on the 'type'
        let query;
        let gametitle;
        if (type == 0) {
            query = 'SELECT * FROM `minutes_1` WHERE phone = ? ORDER BY id DESC LIMIT ?, ?';
            gametitle = "Win Go";  // Game title for type 0
        } else if (type == 1) {
            query = 'SELECT * FROM `result_5d` WHERE phone = ? ORDER BY id DESC LIMIT ?, ?';
            gametitle = "5D";  // Game title for type 1
        } else if (type == 2) {
            query = 'SELECT * FROM `result_k3` WHERE phone = ? ORDER BY id DESC LIMIT ?, ?';
            gametitle = "K3";  // Game title for type 2
        } else {
            // If type is invalid
            return res.status(400).json({
        ...
        // Execute the query with the user phone, pagination params
        const [bet] = await connection.query(query, [userInfo.phone, (page - 1) * limit, limit]);

        // If no bets are found, return an empty list
        let list = (bet.length > 0) ? bet : [];

        // Add the gameTitle to each item in the list
        list = list.map(item => ({
            ...item,           
            gameTitle: gametitle  
        }));
        ...
        console.error('Error fetching user or bet data:', error);
        return res.status(500).json({
            message: error,
            status: false,
            timeStamp: timeNow,
