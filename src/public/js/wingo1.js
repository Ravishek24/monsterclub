...
window.onload = function() {
  function cownDownTimer() {
    var countDownDate = new Date("2030-07-16T23:59:59.9999999+01:00").getTime()+ 30 * 1000;
    ...
    setInterval(() => {
      var now = new Date().getTime()+ 30 * 1000;
      var distance = countDownDate - now;
      var seconds1 = Math.floor((distance % (1000 * 30)) / 10000);
      var seconds2 = Math.floor(((distance % (1000 * 60)) / 1000) % 10);
...
function checkForNewResults() {
    ...
    if (popupCurrentlyShowing) {
        console.log('Popup currently showing, skipping check');
        return;
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (data && data.length > 0) {
                ...
                console.log('Checking popup for:', {
                ...
                if (timeDiff < 30000 && mostRecentGame.stage && !sessionStorage.getItem(popupKey)) {
                    console.log('Showing popup for:', popupKey);
                    ...
                    $.ajax({
                        ...
                        success: function(resultResponse) {
                            ...
                            if (matchingResult) {
                                showWinLossPopup(mostRecentGame, matchingResult, "30sec");
                            }
                        }
                    });
                } else {
                    console.log('Skipping popup - already shown or too old');
                }
            }
        }
    });
...
function showWinLossPopup(gameData, resultData, duration) {
    ...
    if (modal) {
        ...
        setTimeout(() => {
            ...
            // Reset the flag to allow new popups
            popupCurrentlyShowing = false;
            console.log('Popup closed, flag reset');
        }, 5000);
    } else {
        // If modal doesn't exist, reset flag anyway
        popupCurrentlyShowing = false;
        console.log('Modal not found, flag reset');
    }
}
...
function clearPopupHistory() {
    ...
    popupCurrentlyShowing = false;
    console.log('Popup history cleared and flag reset');
}
